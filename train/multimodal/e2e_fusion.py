"""
Python version: 3.10
Description: Trains a Declutr-ViT backbone to perform multimodal authorship identification task using different fusion strategies on Backpage advertisements.
"""
...
parser.add_argument('--nb_negatives', type=int, default=5, help="Number of in-batch negatives")
...
parser.add_argument('--ce_weight', type=float, default=1.0, help="Weightage of CE loss")
parser.add_argument('--supcon_weight', type=float, default=1.0, help="Weightage of SupCon loss")
parser.add_argument('--ntxent_loss', type=float, default=1.0, help="Weightage of NTXent loss")
parser.add_argument('--itm_weight', type=float, default=1.0, help="Weightage of ITM loss")
...
parser.add_argument('--loss', type=str, default='CE', help='Loss function to use. Can be CE, CE+SupCon, or CE+SupCon+ITM')
...
df = map_images_with_text(df).drop_duplicates()
...
print(f"Number of samples in the val dataset: {len(val_dataset)}")
...
# %% Loading the model
model = multimodalFusionModel(text_model=text_model, image_model=image_model, fusion_technique=args.fusion_technique, 
                                    num_classes=len(label_encoder.classes_), learning_rate=args.learning_rate, 
                                    weight_decay=args.weight_decay, eps=args.adam_epsilon, num_training_steps=num_training_steps, 
                                    warmup_steps=warmup_steps, temperature=args.temp, ce_weight=args.ce_weight, supcon_weight=args.supcon_weight, 
                                    itm_weight=args.itm_weight, num_hard_negatives=args.nb_negatives, ntxent_weight=args.ntxent_loss, 
                                    loss_function=args.loss).to(device)
