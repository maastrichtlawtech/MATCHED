"""
Python version: 3.10
Description: Trains an authorship identification and verification baseline with Contrastive loss for vision-based Authorship tasks on Backpage advertisements
"""
...
parser.add_argument('--temp', type=float, default=0.1, help="Temperature coefficient for contrastive learning loss.")
parser.add_argument('--contra_weight', type=float, default=1.0, help="Weightage of contrastive learning loss.")
...
args.num_training_steps = len(data_module.train_dataloader()) * args.nb_epochs
# Setting the warmup steps to 1/10th the size of training data
args.warmup_steps = int(len(data_module.train_dataloader()) * 10/100)

if args.task == "classification":
    sys.path.append('../../architectures/')
    from visionContraLayer import PreTrainedVisionContraModel
    model = PreTrainedVisionContraModel(model_name=args.model_name_or_path, num_classes=num_classes, learning_rate=args.learning_rate,
                                    temp=args.temp, loss2_type=args.loss2_type, nb_negatives=args.nb_negatives, contrastive_weight=args.contra_weight, 
