...
class HTClassifierDataModule(pl.LightningDataModule):
    ...
    def setup(self, stage=None):
        ...
        train_dataset, test_dataset, val_dataset = random_split(dataset, [train_size, test_size, val_size], generator=torch.Generator().manual_seed(args.seed))
...
class HTContraDataModule(pl.LightningDataModule):
    ...
    def load_and_split_data(self):
        # Load the data from a CSV file
        self.data_df = pd.read_csv(self.file_dir)
        
        # Since the vendor IDs are not the curreHTClassifierDataModulent representations of the class labels, we remap these label IDs to 
    ...
    def setup(self, stage='fit'):
        ...
        if stage == 'fit':
            # get dataset
            train_x, train_y = train_df['TEXT'].tolist(), train_df['VENDOR'].tolist()
            test_x, test_y = test_df['TEXT'].tolist(), test_df['VENDOR'].tolist()
            val_x, val_y = val_df['TEXT'].tolist(), val_df['VENDOR'].tolist()

            self.train_set = TokenizeDataset(train_x, train_y, self.tokenizer, self.max_seq_length)
            self.test_set = TokenizeDataset(test_x, test_y, self.tokenizer, self.max_seq_length)
            self.val_set = TokenizeDataset(val_x, val_y, self.tokenizer, self.max_seq_length)
